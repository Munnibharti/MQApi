...
namespace HotelManagementProjectfeb.Repositories
{
...

    {
        ...
        public GuestRepository(HotelManagementDataContext db)
        {
            _db = db;   
        }
          ...
          // here we are deleting basis  on id if id will got then delete other than do not delete
            ...
            {
               var guest = await _db.Reservations.FirstOrDefaultAsync(x => x.Guest_id == id);
        ...
        //here it will show all things
            ...
            {
                  return  await _db.Reservations.FirstOrDefaultAsync(x => x.Guest_id == id);
                

            }
            public async Task<Guest> UpdateGuestAsync(Guid id, Guest guest)
            {
               //here we are making searching is there any id which is existing

               var existingguest=await _db.Reservations.FirstOrDefaultAsync(x => x.Guest_id == id);
